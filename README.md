...
当前版本是 **test12 SM8150 编码缓冲区时序与 Raphael 亮度修正版，待实机验证**。以小米 Android 10
...
分配内部 DMA 缓冲区。编码 DMA 仍默认锁定，自动脚本不再提供打开入口。

同一版本还修复 Raphael 面板的高频亮度更新：亮度 DCS 命令保持 LP 模式，
合并为最多 10 Hz，并在关屏前同步取消待处理任务。该修正针对实机复现的
`dsi_err_worker: status=5`；Mesa 26.1.2 已另外解决音量弹窗触发的 GPU IOVA fault。
...
本轮修正范围见 [test12 说明](docs/venus-test12.md)；原厂、postmarketOS
对照依据保留在 [test4 说明](docs/venus-test4.md)。

源码仍来自 `snowf14k3/linux` 的 `raphael-7.1` 分支，构建时会应用本仓库
`patches/series` 中的十二个补丁（原厂时序、队列校验、诊断、VPU5 会话配置、
...
- `patches.sha256`：十二个补丁各自的 SHA256。
- `venus-test-suite.sh`：安装并启动 test12 后，一次运行的实机测试脚本；
  硬编码受协议和 DMA 两级内核锁保护，默认不会运行；单独设置
  `VENUS_TEST_ENCODER=1` 也只做不提交 DMA 的协议预检，并拒绝 DMA 解锁变量。
