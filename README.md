...
当前版本是 **test13 SM8150 编码缓冲区时序与 Raphael 亮度修正版，待实机验证**。以小米 Android 10
...
分配内部 DMA 缓冲区。test13 保留该修正，并把 test12 无法定位问题的总 DMA
开关替换成 0--9 共十个检查点；同一内核可逐类验证五种内部缓冲，再分别验证
LOAD、START、CAPTURE/FTB 和 INPUT/ETB，默认停在不提交 DMA 的 stage 0。
本轮还按原厂补齐 Annex-B NAL 格式，跳过原厂不会发送的 IRIS1 NV12 stride
属性和零计数 LTRMODE。

同一版本还修复 Raphael 面板的高频亮度更新：请求合并为最多 10 Hz，并在关屏前
同步取消待处理任务。test12 实机确认合并有效，但 LP 亮度命令在 GNOME 重绘期间
仍会产生 DSI timeout 与 FIFO 错误；test13 恢复面板原有的 HS 亮度传输，并保证
传输后无条件恢复 mode flags。Mesa 26.1.2 已另外解决音量弹窗触发的 GPU IOVA fault。
...
本轮修正范围见 [test13 说明](docs/venus-test13.md)；持续更新的原厂逐项核对、
...
`patches/series` 中的十四个补丁（原厂时序、队列校验、诊断、VPU5 会话配置、
...
- `patches.sha256`：十四个补丁各自的 SHA256。
- `venus-test-suite.sh`：安装并启动 test13 后，一次运行的实机测试脚本；
