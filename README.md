...
当前版本是 **test14 SM8150 RC_OFF 编码工作模式与 Raphael 亮度修正版，待实机验证**。以小米 Android 10
...
整机复位。test14 修正原厂 RC_OFF 应使用 WORK_MODE_1 并开启 low-latency 的偏差，
同时保留 0--9 共十个检查点，默认停在不提交 DMA 的 stage 0。
本轮还按原厂补齐 Annex-B NAL 格式，跳过原厂不会发送的 IRIS1 NV12 stride
属性和零计数 LTRMODE。

同一版本还修复 Raphael 面板的高频亮度更新：test13 的 10 Hz HS 传输在纯 sysfs
压力测试中仍产生 DSI status 0x5。test14 恢复 LP 命令并将请求合并为最多 4 Hz，
传输后恢复完整 mode flags。Mesa 26.1.2 已另外解决音量弹窗触发的 GPU IOVA fault。
...
本轮修正范围见 [test14 说明](docs/venus-test14.md)；持续更新的原厂逐项核对、
...
`patches/series` 中的十六个补丁（原厂时序、队列校验、诊断、VPU5 会话配置、
...
- `patches.sha256`：十六个补丁各自的 SHA256。
- `venus-test-suite.sh`：安装并启动 test14 后，一次运行的实机测试脚本；
