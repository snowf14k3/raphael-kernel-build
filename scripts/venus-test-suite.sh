...
if [[ "${1:-}" == --plan ]]; then
	printf '%s\n' \
		'Preflight: test22 kernel, SM8150 Venus nodes, idle devices, tools, free space.' \
...
[[ "$(uname -r)" == *sm8150-venus-test22* ]] || {
	echo '当前不是 test22 内核，未开始测试，也未修改设备设置。' >&2; exit 2;
}
...
if [[ "$scope" == decoder ]]; then
	...
	printf '<6>TEST22_ENCODER_FULL_BEGIN\n' > /dev/kmsg
	logger -t venus-test22-host 'ENCODER_FULL_BEGIN'
	sync
	encode_codec_case encode-h264-one h264_v4l2m2m h264 h264 h264 1 128x96 || stop_batch
	encoder_dma_log="$(dmesg | awk '/TEST22_ENCODER_FULL_BEGIN/{show=1} show')"
